...
void Game::createBoard(std::ifstream & input)
{
	std::string option;
	int dim, w, h;
	float speed;
	while ( input >> option )
	{
		if ( option.find("Tile") != std::string::npos )
		{
			input >> dim;
		}
		if ( option.find("Board") != std::string::npos )
		{
			input >> w >> h;
		}
		if ( option.find("Speed") != std::string::npos )
		{
			input >> speed;
		}
	}
	brd = new Board(gfx, dim, w, h);
	snekSpeedupFactor = new float(speed);
